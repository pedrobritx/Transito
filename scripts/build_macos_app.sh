...
# Ensure PATH includes common Homebrew locations when launched from Finder
DEFAULT_PATHS="/opt/homebrew/opt/ffmpeg/bin:/usr/local/opt/ffmpeg/bin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"
if ! echo "$PATH" | grep -q "/opt/homebrew/bin\|/usr/local/bin"; then
    export PATH="$DEFAULT_PATHS:$PATH"
fi

# Minimal logging to help diagnose PATH issues when launched from Finder
LOG_FILE="$HOME/Library/Logs/Transito.launch.log"
{
  echo "[Transito] Launch at $(date)"
  echo "[Transito] PATH=$PATH"
  echo -n "[Transito] which ffmpeg: "; which ffmpeg || true
  echo -n "[Transito] which ffprobe: "; which ffprobe || true
} >> "$LOG_FILE" 2>&1
...
# Launch the GUI with auto-install enabled
cd "$RESOURCES_DIR"
export HLS_DOWNLOADER_AUTO_INSTALL=1
